...
class AAFWriterTests(unittest.TestCase):
    ...
    def test_transcribe_embed_wav_audio(self):
        """Tests simple WAV audio essence import."""
        range = otio.opentime.TimeRange(
            start_time=otio.opentime.RationalTime(
                value=0,
                rate=24.0
        ...
        media_reference_wav = otio.schema.ExternalReference(
            target_url=os.fspath(Path(SAMPLE_DATA_DIR) / "picchu_sample.wav"),
            available_range=range
        )
        wav_clip = otio.schema.Clip(
            name="EmbeddedClip",
            source_range=range,
            media_reference=media_reference_wav
        )

        track = otio.schema.Track(children=[wav_clip], kind=otio.schema.TrackKind.Audio)
        tl = otio.schema.Timeline(tracks=[track])

        _, tmp_aaf_path = tempfile.mkstemp(prefix="embed_wav_", suffix='.aaf')
